...
export async function fetchRecentActivities(limit: number = 5): Promise<ActivityItem[]> {
  ...
  try {
    ...
    // Check if the activities table exists by querying for table existence
    // But for now, we'll fall back to mock data since the table doesn't exist yet
    console.log('[RecentActivities] Using mock data since activities table does not exist in schema');
