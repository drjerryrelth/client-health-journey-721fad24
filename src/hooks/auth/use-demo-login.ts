...
export const useDemoLogin = () => {
    ...
    try {
      ...
      try {
        ...
        if (accountExists) {
          ...
          await navigateToRoleDashboard(type);
          return;
        }
        ...
        try {
          ...
          handleSignupErrors(signupError, email, password, type);
        }
      }
  ...
  // Helper function to handle navigation after login based on role
  const navigateToRoleDashboard = async (type: UserRole) => {
    return new Promise<void>(resolve => {
      // Wait briefly to ensure auth context is updated
      setTimeout(() => {
  ...
  const handleSignupErrors = async (signupError: any, email: string, password: string, type: UserRole) => {
